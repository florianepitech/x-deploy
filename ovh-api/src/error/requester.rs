...
impl OvhClient {
    pub(crate) async fn send_get_mapped<T>(
        &self,
        url: &str,
    ) -> Result<serde_json::Value, OvhClientError> where T: serde::de::DeserializeOwned {
    ...
    pub(crate) async fn send_mapped<T>(
        &self,
        url: &str,
    ) -> Result<T, OvhClientError> where T: serde::de::DeserializeOwned {
        ...
        if response_status_code != 200 {
            return Err(OvhClientError::WrongResponse(response, response_status_code));
        }
