...
impl OvhClient {
    ...
    pub(crate) async fn send_get_request(&self, url: &str) -> Result<Response, Error> {
        let url: Url = Url::parse(&url).unwrap();
        let timestamp: i64 = chrono::Utc::now().timestamp();
        let signature =
            self.create_signature("GET", url.to_string().as_str(), "", &timestamp.to_string());
        let base_request = reqwest::Request::new(reqwest::Method::GET, url);
        let request =
            reqwest::RequestBuilder::from_parts(self.reqwest_client.clone(), base_request)
