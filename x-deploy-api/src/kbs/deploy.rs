...
pub async fn deploy(deployment_info: DeployInfo) -> &'static str {
  let client = Arc::new(OvhClient::new(
    std::env::var("OVH_APPLICATION_KEY")
      .expect("OVH_APPLICATION_KEY not found"),
    std::env::var("OVH_APPLICATION_SECRET")
      .expect("OVH_APPLICATION_SECRET not found"),
    std::env::var("OVH_CONSUMER_KEY").expect("OVH_CONSUMER_KEY not found"),
  ));
  let kubeconfig = ovh_api::route::cloud::get_kubconfig(
    &client,
  ...
  let kube_client =
    kbs::connect_with_kubeconfig(kubeconfig.content.as_str()).await;
  let deployment = create_deployment_type(deployment_info.clone());
  let deployments: Api<Deployment> =
    Api::namespaced(kube_client, &deployment_info.namespace);
